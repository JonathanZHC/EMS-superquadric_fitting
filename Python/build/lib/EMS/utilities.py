...
def read_ply(filename):
    """
    Read a PLY file and return Nx3 numpy array (x,y,z).
    Works even if PLY contains extra fields (colors, normals, etc.)
    """
    ...
    if not all(k in vertex.data.dtype.names for k in ['x', 'y', 'z']):
        raise ValueError("PLY file does not contain x, y, z fields.")
